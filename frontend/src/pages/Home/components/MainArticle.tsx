...
import { isRecent } from '../utilities/is-recent.utility'

export function MainArticle({ article }: { article?: ArticleData }) {
  ...
  return (
    <Article id={article.id} className='flex flex-col gap-y-4 w-full relative'>
      {isRecent(article.createdAt) && (
        <div className='absolute m-3 bg-brand-red text-white text-sm font-bold px-3 py-1 rounded-sm z-10'>
          NUEVO
        </div>
      )}
      <img
