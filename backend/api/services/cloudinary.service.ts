...
export class CloudinaryService {
  static async upload(fileBuffer: Buffer, filename: string, folder = "articles") {
  ...
  static async updateImage({ file, article, body }: { article: Article, file?: Express.Multer.File, body: any }) {
    if (file) {
      if (article.image) {
      ...
      const uploadResult = await CloudinaryService.upload(file.buffer, article.title);
